...
export class NetworkPeer extends Peer {
    ...
    private _onBroadcast(str: Buffer | string, peerAddress: PeerAddress): void {
        ...
        // TODO delete crutch after update all nodes
        let response: { code: string, data: any };
        if (Buffer.isBuffer(str)) {
            response = deserialize(str);
            console.log(`[Model][NetworkPeer][onBroadcast] code: ${response.code}, size: ${str.length} bytes`);
        } else {
            response = new SocketResponse(str);
            console.log(`[Model][NetworkPeer][onBroadcast] code: ${response.code} length: ${str.length} is JSON`);
        }
    ...
    private _onRPCRequest(str: Buffer | string): void {
        ...
        // TODO delete crutch after update all nodes
        let response: { code: string, data: any, requestId: string };

        if (Buffer.isBuffer(str)) {
            const serializer = new SocketBufferRPC();
            response = serializer.unpack(str);
        } else {
            response = new SocketResponseRPC(str);
        }
